...
The SCMC method implies interpolating $F\_{Y}^{-1}(F\_{X}(x))$, where
$Y$ is the target random variable and $X$ is a random variable which can
be efficiently generated, and generating the samples $y\_{i}, i=1,,n$
using the formula $y\_{i} = F\_{Y}^{-1}(F\_{X}(*{i}))$, where $*{i}$ are
...
inverse $F\_{Y}^{-1}$ (i.e. the quantile function of $Y$) as the first
...
$f\_{X}(x)$ (density of $X$). The third argument to `univariate_sampler`
